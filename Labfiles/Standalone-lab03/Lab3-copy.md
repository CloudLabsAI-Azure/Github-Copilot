...
1. On the **Sign in to GitHub** tab, you will see the login screen. In that screen, enter the following **email: odl-user-<inject key="Deployment-id" enableCopy="false"/>_clabs**. or use <inject key="GitHub User Name" enableCopy="true"/> Then click on **Sign in with your identity provider** **(2)**. 
